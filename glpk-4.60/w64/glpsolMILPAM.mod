...
set Ns;/*物理节点的集合*/
set Nss{k in Ns};/*物理节点k对应的链路的后驱节点集合*/
...
/*minimize slotsMin: sum{(i,j) in Lv}sum{m in MD}(S[i,j,m])/m;*/
/*minimize slotsMin: 1000*sum{i in Nv}sum{j in X[i]}sv[i,j]*A[i,j]*p[i]+1*sum{(i,j) in Lv}sum{(m,n) in Ls}sum{a in SlotIndex}sum{md in MD}cs[m,n]*((pbw[i,j]/(12.5*md)+1)+G)*Z[i,j,m,n,a,md];*/

minimize slotsMin: 1*sum{i in Nv}sum{j in X[i]}sv[i,j]*A[i,j]+1*sum{(i,j) in Lv}sum{(m,n) in Ls}sum{a in SlotIndex}sum{md in MD}cs[m,n]*((pbw[i,j]/(12.5*md)+1)+G)*Z[i,j,m,n,a,md]+1*sum{i in Nv}sum{k in Ns}c[k]*p[i]*A[i,k];
/*minimize slotsMin: sum{i in Nv}sum{j in X[i]}(sv[i,j])*A[i,j];*/

/*minimize slotsMin:sum{i in Nv,j in Ns} A[i,j]*K[j] + sum{(i,j) in Lv}sum{(m,n) in Ls}sum{a in SlotIndex}sum{md in MD}cs[m,n]*((pbw[i,j]/(12.5*md)+1)+G)*Z[i,j,m,n,a,md]+sum{i in Nv}sum{k in Ns}c[k]*p[i]*A[i,k];*/
