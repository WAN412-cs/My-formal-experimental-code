...
minimize slotsMin:sum{i in Nv}sum{k in X[i]}(ac[i,k])*A[i,k];

/*minimize slotsMin: sum{(i,j) in Lv}sum{(m,n) in Ls}sum{a in SlotIndex}sum{md in MD}cs[m,n]*((pbw[i,j]/(12.5*md)+1)+G)*Z[i,j,m,n,a,md]+ sum{i in Nv}sum{k in X[i]}(ac[i,k])*A[i,k];*/
