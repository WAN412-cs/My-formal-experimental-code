...
param c{u in Nv,v in Af[u]};/*分配CPU的单价*/
param cr{u in Nv};/*虚拟节点u请求的cpu资源量*/
...
*/
var AP{u in Nv,v in Af[u]},>=0;/*分配的CPU数量*/

minimize slotsMinAndTransCostMin: sum{d in D}sum{p in Path} NSd[d,p]*y[d,p]*(H[p]-2)+sum{u in Nv,v in Af[u]} AP[u,v]*c[u,v];

/****节点分配的CPU约束*****/
s.t. NodeAssCPU{u in Nv,v in Af[u]}:AP[u,v]=cr[u]*M[u,v];
