...
/*s.t. SlotOfLinkConstraint2{p in MBSet,(i,j) in E}: (sum{k in BMatch[p]} f[i,j,k])<=(sum{q in BSet}y[i,j,q]);*/
/* Single link continue constraints of slots (2.2).*/ 

/*s.t. SlotOfLinkConstraint3{p in MBSet,(i,j) in E}: (sum{q in BSet}y[i,j,q]) = x[i,j]*B;*/
/* Single link continue constraints of slots (2.3).*/ 
...
/*s.t. SlotOfLinkConstraint6{(i,j) in E}: zx[i,j]+B<=(M-1);*/
/*s.t. SlotOfLinkConstraint6:c<=(M-1);*/
/*s.t. SlotOfLinkConstraint7{(i,j) in E}: zx[i,j]+B<=c-1;*/
/*s.t. SlotOfLinkConstraint8{(i,j) in E,k in MSet}: f[i,j,k]*k<=c;*/
/*s.t. SlotOfLinkConstraint9{(i,j) in E,k in MSet}: f[i,j,k]*k>=zx[i,j];*/
/*s.t. SlotOfLinkConstraint10{(i,j) in E,k in MSet:k<c}: f[i,j,k]*k>=x[i,j]*k;*/
end;