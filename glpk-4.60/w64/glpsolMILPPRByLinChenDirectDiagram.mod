...
param R;/*最大的整数*/
param G;/*隔离槽数量*/
param ac{i in Nv,j in X[i]};/*虚拟节点映射到物理节点的分配代价*/
...
/*minimize slotsMin:sum{i in Nv}sum{k in X[i]}(ac[i,k])*p[i]*A[i,k]+sum{(i,j) in Lv}sum{(m,n) in Ls}sum{a in SlotIndex}sum{md in MD}cs[m,n]*((pbw[i,j]/(12.5*md)+1)+G)*Z[i,j,m,n,a,md]+sum{i in Nv}sum{k in Ns}c[k]*p[i]*A[i,k];*/

minimize slotsMin:sum{i in Nv}sum{k in X[i]}(ac[i,k])*A[i,k];/*p[i]*/
/*minimize slotsMin: sum{(i,j) in Lv}sum{(m,n) in Ls}sum{a in SlotIndex}sum{md in MD}cs[m,n]*((pbw[i,j]/(12.5*md)+1)+G)*Z[i,j,m,n,a,md]+sum{i in Nv}sum{k in Ns}c[k]*p[i]*A[i,k];*/
/*s.t.a1:A[0,4]=1;
s.t.a2:A[1,2]=1;
s.t.a3:A[2,3]=1;
s.t.a4:A[3,1]=1;
s.t.a5:M[0,2,4,3,0]=1;
s.t.a6:M[0,1,4,2,0]=1;
s.t.a7:M[0,3,4,1,0]=1;*/


/*minimize slotsMin: sum{(i,j) in Lv}sum{(m,n) in Ls}sum{a in SlotIndex}sum{md in MD}cs[m,n]*((pbw[i,j]/(12.5*md)+1)+G)*Z[i,j,m,n,a,md]+ sum{i in Nv}sum{k in X[i]}(1000000*ac[i,k])*A[i,k] +sum{i in Nv}sum{k in Ns}c[k]*p[i]*A[i,k];*/
...
s.t. NetResAssignmentCon2{(i,j) in Lv,(m,n) in Ls,a in SlotIndex,md in MD}: (pbw[i,j]/(12.5*md)+G)*Z[i,j,m,n,a,md]<=sum{t in Slot[i,j,a,md]}USlot[m,n,t];/*公式14*/
...
s.t.aa3{(i,j) in Lv,a in SlotIndex,(m,n) in Ls}:M[i,j,m,n,a]+M[i,j,n,m,a]<=1;
s.t.aa31{(i,j) in Lv,a in SlotIndex,(m,n) in Ls,md in MD}:Z[i,j,m,n,a,md]+Z[i,j,n,m,a,md]<=1;

s.t. NetResAssignmentCon32{(m,n) in Ls,a in SlotIndex}: sum{(i,j) in Lv}sum{md in MD}sum{t in Slot1[i,j,a,md]}Z[i,j,m,n,t,md]<=1;/*公式15*/
s.t. NetResAssignmentCon33{(m,n) in Ls}: sum{(i,j) in Lv}sum{a in SlotIndex}(M[i,j,m,n,a]+M[i,j,n,m,a])<=1;/*公式15*/

s.t.aa4{(i,j) in Lv,a in SlotIndex,m in Ns}:sum{(m,n) in Ls}M[i,j,m,n,a]<=1;
s.t.aa5{(i,j) in Lv,a in SlotIndex,m in Ns}:sum{(n,m) in Ls}M[i,j,n,m,a]<=1;

s.t.aa6{(n,m) in Ls,a in SlotIndex}:sum{(i,j) in Lv}M[i,j,n,m,a]<=1;

s.t. NetResAssignmentCon31{(m,n) in Ls,a in SlotIndex,md in MD,t in SlotIndex}: sum{(i,j) in Lv}Z[i,j,m,n,t,md]<=USlot[m,n,a];/*公式15*/
