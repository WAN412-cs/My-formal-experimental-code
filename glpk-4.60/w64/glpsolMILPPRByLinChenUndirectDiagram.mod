...
s.t. NetResAssignmentCon32{(m,n) in Ls,a in SlotIndex}: sum{(i,j) in Lv}sum{md in MD}sum{t in Slot1[i,j,a,md]}Z[i,j,m,n,t,md]<=1;
s.t. NetResAssignmentCon33{(m,n) in Ls}: sum{(i,j) in Lv}sum{a in SlotIndex}(M[i,j,m,n,a]+M[i,j,n,m,a])<=1;
*/
...
s.t. NetResAssignmentCon31{(m,n) in Ls,a in SlotIndex,md in MD,t in SlotIndex}: sum{(i,j) in Lv}Z[i,j,m,n,t,md]<=USlot[m,n,a];
/**/
