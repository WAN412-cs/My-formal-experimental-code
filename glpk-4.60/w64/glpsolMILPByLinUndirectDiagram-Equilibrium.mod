...
minimize slotsMin: sum{(i,j) in Lv}sum{(m,n) in Ls}sum{a in SlotIndex}sum{md in MD}((pbw[i,j]/(12.5*md)+1)+G)*Z[i,j,m,n,a,md]+sum{i in Nv}sum{k in X[i]}costCPU[k,i]*A[i,k];
/*minimize slotsMin: sum{(i,j) in Lv}sum{(m,n) in Ls}sum{a in SlotIndex}sum{md in MD}cs[m,n]*((pbw[i,j]/(12.5*md)+1)+G)*Z[i,j,m,n,a,md]+sum{i in Nv}sum{k in X[i]}costCPU[k,i]*A[i,k];*/

/*minimize slotsMin: sum{i in Nv}sum{k in Ns}(ac[k,i])*A[i,k];*/
...
s.t.a2:A[1,0]=1;
s.t.a3:A[2,3]=1;
s.t.a4:M[0,2,1,3,4]=1;
s.t.a5:M[0,1,1,0,4]=1;*/
/*s.t.a2:A[0,4]=1;*/
/*s.t.a3:A[2,5]=1;*/
