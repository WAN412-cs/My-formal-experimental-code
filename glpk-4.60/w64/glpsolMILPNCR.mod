...
minimize slotsMin: 10*sum{i in Nv}sum{j in X[i]}sv[i,j]*A[i,j]*p[i]+1*sum{(i,j) in Lv}sum{(m,n) in Ls}sum{a in SlotIndex}sum{md in MD}cs[m,n]*((pbw[i,j]/(12.5*md)+1)+G)*Z[i,j,m,n,a,md]+1*sum{i in Nv}sum{k in Ns}c[k]*p[i]*A[i,k];
